...
class SuppliesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create supply" do
    assert_difference('Supply.count') do
      post supplies_url, params: { supply: { date: @supply.date, status: @supply.status, supplier_id: @supply.supplier_id } }
    end
  ...
  test "should update supply" do
    patch supply_url(@supply), params: { supply: { date: @supply.date, status: @supply.status, supplier_id: @supply.supplier_id } }
